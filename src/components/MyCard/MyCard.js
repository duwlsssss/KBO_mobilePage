...
function MyCard() {
    ...
    api
      ...
      .then((response) => {
        ...
        if (data.status === "ok" && data.data.length > 0) {
          ...
          const userCards = data.data.filter(
            (card) => card.userEmail === userEmail
          );
  ...
  useEffect(() => {
    ...
    loadData();
  }, [userEmail, fetchCards, fetchImages]); // userEmail이 변경될 때마다 이 효과를 다시 실행
  ...
  }, [location, setUserEmail]); //컴포넌트가 마운트될 때, userEmail 추출하고 이를 zustand에 저장
  ...
  const { backImageUrl, frontImageUrl } = useMemo(() => {
    if (cards.length > 0) {
      const card = cards[0];
      return {
        backImageUrl: card.backgroundOption
          ? getBackImageUrl(card.backgroundOption)
          : "/images/back/GreyA.webp",
        frontImageUrl: card.backgroundOption
          ? `/images/front/${card.backgroundOption}.webp`
          : "/images/front/GreyAurora.webp",
      };
    }
  ...
  useEffect(() => {
    if (cards.length > 0) {
      ...
      setPatternUrl(
        card.patternOption ? `/images/pattern/${card.patternOption}.webp` : ""
      );
      setFrameUrl(
        card.frameOption ? `/images/frame/${card.frameOption}.webp` : ""
      );
    ...
    return (
      <div className={styles.popUp}>
        <div className={styles.popUpContent}>
          <div>카드 로드 중...</div>
          <ProgressBar progressDuration={4000} totalBlocks={16} />{" "}
          {/* 여기서 넘기는 초가 더 적어야 progressBar가 먼저 사라지지 않음 */}
        </div>
      </div>
    );
  }
      ...
      <div className={styles.container}>
        <div className="title-bar">
          <span className={styles.addressText}>
            https://www.kimsoffice.com :: 김씨네{" "}
            <span className={styles.addressTextColor}> 명함제작 </span> 사무소
          </span>
          <div
        ...
        <div className={styles.contentArea}>
          {isSaving && (
            <div className={styles.popUp}>
              <div className={styles.popUpContent}>
                <div>사진 저장 중...</div>
                <div>
                  <ProgressBar progressDuration={1000} totalBlocks={16} />
                </div>
              </div>
            </div>
          )}
          {cards.length > 0 ? (
            <>
              <div className={styles.ownerText}>
                <span className={styles.ownerTextStrong}>
                  {cards[0].name || "사용자"}
                </span>{" "}
                님의 명함
              </div>
              <div className={styles.card}>
                <div
                  ...
                  style={cardStyles.front}
                  ref={frontRef}
                >
                  <div className={styles.infoContainer}>
                    <div className={styles.date} style={infoItemStyle}>
                      {cards[0].updatedAt
                        ? new Date(cards[0].updatedAt).toLocaleDateString()
                        : ""}
                    </div>
                    <div className={styles.name} style={infoItemStyle}>
                      이름
                    </div>
                    <div className={styles.nameValue} style={infoItemStyle}>
                      {cards[0].name || ""}
                    </div>
                    <div className={styles.engNameValue} style={infoItemStyle}>
                      {cards[0].engName || ""}
                    </div>
                    <div className={styles.school} style={infoItemStyle}>
                      학교
                    </div>
                    <div
                      className={
                    ...
                    <div
                      className={styles.studentNumValue}
                      style={infoItemStyle}
                    >
                      {cards[0].studentNum || ""}
                    </div>
                    <div className={styles.major} style={infoItemStyle}>
                      전공
                    </div>
                    <div className={styles.majorValue} style={infoItemStyle}>
                      {cards[0].major || ""}
                    </div>
                    <div className={styles.session} style={infoItemStyle}>
                      진로
                    </div>
                    <div className={styles.sessionValue} style={infoItemStyle}>
                      <span className={styles.highlight}>
                        {cards[0].session || ""}
                      </span>
                    </div>
                    <div className={styles.MBTI} style={infoItemStyle}>
                      MBTI
                    </div>
                    <div className={styles.email} style={infoItemStyle}>
                      이메일
                    </div>
                    <div
                    ...
                    <div className={styles.IG} style={infoItemStyle}>
                      IG
                    </div>
                    <div
                      className={styles.IGValue}
                      style={infoItemStyle}
                      onClick={handleIgClick}
                    >
                      {cards[0].ig ? `@${cards[0].ig}` : ""}
                    </div>
                    {frameUrl && (
                      <img
                    ...
                    {cardImage && (
                      <div
                        className={
                          cards[0].frameShapeoption === "Rec"
                            ? styles.cardImageContainerR
                            : styles.cardImageContainerC
                        }
                      >
                        <img
                          src={cardImage}
                          ...
                          className={
                            cards[0].frameShapeoption === "Rec"
                              ? styles.cardImageRectGrey
                              : styles.cardImageCircleGrey
                          }
                        />
                      </div>
                    )}
                ...
                >
                  ...
                  <div className={styles.moto} style={infoItemStyle}>
                    {cards[0].moto || ""}
                  </div>
                  {showQR && (
                    <div className={styles.QR}>
                      <QRCode
                        value={`https://kimsofficebc.netlify.app/card-info?userEmail=${userEmail}`}
                        size={30}
                      />
                    </div>
                  )}
          ...
          <div className={styles.btnContainer}>
            ...
            <button
              type="button"
              className={styles.btn}
              onClick={saveCardBAsImage}
            >
              뒤저장
            </button>
            <button type="button" className={styles.btn} onClick={shareCard}>
              공유
            </button>
          </div>
        </div>
